...
// Meshtastic types
interface MeshNode {
  id: string; shortName: string; longName: string
  rssi: number; snr: number; hops: number
  battery: number; voltage: number
  lat: number; lng: number; alt: number
  lastHeard: string; uptime: string
  isMqttGateway: boolean; isOnline: boolean
}
interface MeshMessage {
  id: number; from: string; fromShort: string
  to: string; channel: number
  type: 'TEXT' | 'POSITION' | 'NODEINFO' | 'TELEMETRY' | 'ROUTING'
  text?: string; time: string; ack: boolean; hops: number
}
interface MeshChannel {
  index: number; name: string
  role: 'PRIMARY' | 'SECONDARY' | 'DISABLED'
  psk: string; uplinkEnabled: boolean; downlinkEnabled: boolean
}
...
const MESH_CHANNELS_INIT: MeshChannel[] = [
...
function TopBar({ section, callsign, onCallsignChange }: { section: NavSection; callsign: string; onCallsignChange: (v: string) => void }) {
  ...
  const [conn, setConn] = useState('Icom USB Control (CI-V)')
...
function RadioSection() {
  const [freqKhz, setFreqKhz] = useState(14200)
  const [subFreqKhz, setSubFreqKhz] = useState(7074)
  const [mode, setMode] = useState('USB')
  const [subMode, setSubMode] = useState('FT8')
  const [band, setBand] = useState('20m')
  const [txMode, setTxMode] = useState(false)
  const [sMeter, setSMeter] = useState(7)
  const [power, setPower] = useState(0)
  const [agc, setAgc] = useState('MID')
  const [att, setAtt] = useState('OFF')
  const [nb, setNb] = useState(false)
  const [nr, setNr] = useState(false)
  const [split, setSplit] = useState(false)
  const [vfoA, setVfoA] = useState(true)
  const [rfGain, setRfGain] = useState(80)
  const [afGain, setAfGain] = useState(60)
